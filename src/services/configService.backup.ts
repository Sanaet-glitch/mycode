...
export const getEmailTemplateById = async (id: string): Promise<EmailTemplate | null> => {
  try {
    ...
    console.error(`Error retrieving email template ${id}:`, error);
    return null;
  }
};
...
export const createEmailTemplate = async (
  template: Omit<EmailTemplate, 'id' | 'created_at' | 'updated_at' | 'last_used_at'>
): Promise<string> => {
  try {
    const { data, error } = await supabase
      .from('email_templates')
...
export const createBackup = async (
  ...
  try {
    ...
    // Now initiate the actual backup process
    // This would typically be done via a Supabase Edge Function or similar
    const { error: functionError } = await supabase.functions.invoke('create-backup', {
      body: {
        backupId,
        includeDatabase,
        includeFiles
  ...
  try {
    ...
    if (data) {
      return {
        ...
        databaseConnection: data.database_status,
        uptime: data.uptime,
    ...
    // Fallback to default data if none is found
    return {
      version: '1.0.0',
